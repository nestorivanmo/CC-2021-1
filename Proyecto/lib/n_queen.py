...
def remove_population_redundancy(q):
    population = q.get()
...
def mutation2(redundant_generation, num_processes=mp.cpu_count()):
    ...
    for i in range(num_processes):
        q = mp.Queue()
        q.put(splitted_children[i])
        proc = mp.Process(target=remove_population_redundancy, args=(q,))
        proc.start()
        procs.append(proc)
        queues.append(q)
    for proc in procs:
        proc.join()
    offspring = queues[0].get()
    for i in range(1, len(queues)):
        offspring = np.concatenate((offspring, queues[i].get()))
...
if __name__ == '__main__':
    ...
    # for i in range(10):
    #     start = time()
    #     solutions, iterations, sol_process_id = genetic_algorithm_mp(50, 50, mp.cpu_count())
    #     end = time()
    #     times.append(end-start)
    #     print(solutions, iterations)
    #     #print(solutions, iterations, sol_process_id, end-start)
    # print(np.mean(times))
